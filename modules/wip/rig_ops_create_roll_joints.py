...
class RigOps_CreateRollJoints(object):
    ...
    def create_twist_joints(
        twistJointCount=twistJointCount, Init_JNT0=bn1, Init_JNT1=bn2, Init_JNT2=bn3
    ):
        ...
        twist_joint_rtm_ik_1 = mc.joint(
            n="{}_RTM_Root_Twist_BN_JNT".format(name), rad=JointSize * 2
        )
        twist_joint_rtm_ik_2 = mc.joint(
            n="{}_RTM_Mid_Twist_JNT".format(name), rad=JointSize * 2
        )
        mc.parent(twist_joint_rtm_ik_1, rigGroup)
        mc.select(cl=True)
        twist_joint_ETM_ik_1 = mc.joint(
            n="{}_ETM_End_Twist_BN_JNT".format(name), rad=JointSize * 2
        )
        twist_joint_ETM_ik_2 = mc.joint(
            n="{}_ETM_Mid_Twist_JNT".format(name), rad=JointSize * 2
        )
        ...
        for i in range(twistJointCount):
            ...
            joint_location = (
                root_to_mid_vec - ((root_to_mid_vec / tJC) * rtm_multiple)
            ) + root_twist_vec
        ...
        for i in range(twistJointCount):
            ...
            # Cycle through the positions
            joint_location = (
                mid_to_end_vec - ((mid_to_end_vec / tJC) * ETM_multiple)
            ) + mid_twist_vec
        ...
        RTM_IKS_Handle = mc.ikHandle(
            name="{}_RTM_IKS".format(name),
            sol="ikSCsolver",
            sj=twist_joint_rtm_ik_1,
            ee=twist_joint_rtm_ik_2,
        )[0]
        ETM_IKS_Handle = mc.ikHandle(
            name="{}_RTM_IKS".format(name),
            sol="ikSCsolver",
            sj=twist_joint_ETM_ik_1,
            ee=twist_joint_ETM_ik_2,
        )[0]
        ...
        for i in rtm_twist_joints:
            ...
            oriConst = mc.orientConstraint(
                bn1, twist_joint_rtm_ik_1, i, mo=True, skip=["y", "z"]
            )[0]
        ...
        for i in ETM_twist_joints:
            ...
            oriConst = mc.orientConstraint(
                bn3, twist_joint_ETM_ik_1, i, mo=True, skip=["y", "z"]
            )[0]
        ...
        for i in range(len(etm_constraints)):
            mc.setAttr((etm_constraints[i] + "." + bn3 + "W0"), 1 + (i - 0.5))
